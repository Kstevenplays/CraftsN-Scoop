...
@Component({
  ...
  styles: [
    ...
    `.timeline li{position:relative;padding-top:1rem;text-align:center;color:#6b7280}`,
    `.timeline li:not(:last-child)::after{content:'';position:absolute;left:50%;top:.3rem;width:100%;height:2px;background:#d1d5db;z-index:0}`,
    `.dot{position:absolute;left:50%;top:0;transform:translateX(-50%);width:.7rem;height:.7rem;border-radius:999px;background:#d1d5db;border:2px solid #fff;z-index:1}`,
    `.timeline li.complete{color:#065f46}`,
    `.timeline li.complete .dot{background:#10b981}`,
    `.timeline li.current{color:#1d4ed8}`,
    `.timeline li.current .dot{background:#3b82f6}`,
    `.timeline.is-cancelled li{color:#9ca3af}`,
    `.timeline.is-cancelled .dot{background:#d1d5db}`,
